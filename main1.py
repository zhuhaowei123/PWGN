...
import json
import os
import data.PWGN.models as models
import data.PWGN.dataset as datasets
import data.PWGN.util as util
import data.PWGN.denoise as denoise
import torch
...
def get_arguments():
    ...
    config='D:/project/PWGN/data/PWGN/config.json'
    mode='inference'
    load_checkpoint='D:/project/PWGN/data/NSDTSEA/checkpoints/config1_epoch0015.pth'
    condition_value=0
    batch_size=10
    one_shot=False
    clean_input_path='D:/project/PWGN/data/NSDTSEA/clean_speech/'
    noisy_input_path='D:/project/PWGN/data/NSDTSEA/noisy_speech/'
...
def get_dataset(config, model):
    if config['dataset']['type'] == 'nsdtsea':
...
def training(config, args):
    ...
    # Instantiate Model
    model = models.DenoisingPWGN(config)
    print('model loaded...')
...
def inference(config, args):
    ...
    if not bool(args['one_shot']):
        model = models.DenoisingPWGN(config)
        # model = models.DenoisingPWGN(config, target_field_length=args['target_field_length'])
    ...
    if args['noisy_input_path'].endswith('.mat'):
        ...
        filenames = [filename for filename in os.listdir(args['noisy_input_path']) if filename.endswith('.mat')]

    clean_input = None
    for filename in filenames:
        # noisy_input = util.load_wav(args['noisy_input_path'] + filename, config['dataset']['sample_rate'])
        noisy_input = scipy.io.loadmat(args['noisy_input_path'] + filename)['result']
        # if args['clean_input_path'] is not None:
        #     # clean_input = util.load_wav(os.path.join(args['clean_input_path'], filename), config['dataset']['sample_rate'])
        #     clean_input = scipy.io.loadmat(args['clean_input_path'] + filename)['result']
        ...
        #     model = models.DenoisingPWGN(config, input_length=len(inputs['noisy']))

        print("Denoising: " + filename)
        
        predict_config = models.PredictConfig(model, args['load_checkpoint'])
        denoise.denoise_sample(predict_config, inputs, condition_input, batch_size, output_filename_prefix, output_folder_path)
